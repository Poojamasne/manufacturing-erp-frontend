...
const OpportunityList: React.FC = () => {
  ...
  const opportunityLeads = useMemo(() => {
    if (!leads) return [];
    // Filter leads to only show those with status in OPPORTUNITY_STATUSES
  ...
  const getStatusStyle = (status: string) => {
    ...
    switch (status) {
      ...
      case "Contacted":
        return base + "bg-cyan-50 text-cyan-600 border-cyan-100";
      default:
                  ...
                  <div className="absolute right-0 mt-2 w-full bg-white border border-slate-100 rounded-2xl shadow-2xl z-50 py-2">
                    {statusOptions.map((opt) => (
                      <button
                        ...
                        className={`outline-none w-full text-left px-4 py-2 text-[13px] hover:bg-slate-50 ${statusFilter === opt ? "text-[#F59E0B] font-bold bg-teal-50/50" : "text-slate-600"}`}
                      >
                        {opt}
                      </button>
                    ))}
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-4 py-4 text-center">
                        <span
                          className={`font-bold ${getValueStyle(opportunityValue)}`}
                        >
                      ...
                      <td className="px-4 py-4 text-center">
                        <span className={getPriorityStyle(lead.priority)}>
                          {lead.priority}
                        </span>
                      </td>
                      <td className="px-4 py-4">
                        <div className="flex justify-center gap-2">
                          <button
                            onClick={() =>
                              navigate(
                                "/sales/opportunities/opportunity-view/" +
                                lead.id,
                              )
                            }
                          ...
                          <button
                            onClick={() =>
                              navigate(
                                "/sales/opportunities/opportunity-edit/" +
                                lead.id,
                              )
                            }
            ...
            {!loading && filteredOpportunities.length === 0 && (
              <div className="py-32 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-slate-400 text-sm max-w-xs">
                  No qualified leads available. Opportunities are created when
                  leads are marked as Qualified, Won, or In Progress.
                </p>
              </div>
            )}
          </div>
              ...
              <div className="flex items-center gap-6">
                <div className="text-[11px] font-bold text-slate-800 uppercase tracking-widest">
                  ...
                  <span className="text-slate-900">
                    {filteredOpportunities.length > 0 ? startIndex + 1 : 0}
                  </span>{" "}
                  to{" "}
                  <span className="text-slate-900">
                    {Math.min(endIndex, filteredOpportunities.length)}
                  </span>{" "}
                  of{" "}
                  <span className="text-slate-900">
