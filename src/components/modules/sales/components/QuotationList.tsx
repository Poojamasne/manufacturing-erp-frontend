...
const QuotationList: React.FC = () => {
  ...
  const { quotations, error } = useAppSelector(
  ...
  const filteredQuotations = useMemo(() => {
    ...
    return quotationsArray.filter((qt: Quotation) => {
      ...
      const quoteDate = qt.quotation_date
        ? new Date(qt.quotation_date)
        : new Date();
      const now = new Date();
      ...
      if (activeTab === "Custom") {
        ...
        const diffInDays =
          (now.getTime() - quoteDate.getTime()) / (1000 * 60 * 60 * 24);
        if (activeTab === "Weekly")
          matchesTime = diffInDays <= 7 && diffInDays >= 0;
        if (activeTab === "Monthly")
          matchesTime =
            quoteDate.getMonth() === now.getMonth() &&
            quoteDate.getFullYear() === now.getFullYear();
        if (activeTab === "Yearly")
          matchesTime = quoteDate.getFullYear() === now.getFullYear();
      }
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-10">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1 font-medium">
              Draft, track and manage client commercial proposals.
            </p>
          </div>

          {/* Button Container - Aligned in a row */}
          <div className="flex items-center gap-3">
            ...
            <button
              ...
              className="outline-none flex items-center gap-1 bg-[#F59E0B] hover:bg-[#f67317] text-white px-2.5 py-2 rounded-xl font-bold text-sm shadow-lg shadow-orange-900/10 transition-all active:scale-95 whitespace-nowrap"
            >
              <Plus size={18} />
              Create Quotation
            </button>
          ...
          <div className="p-6 flex flex-col lg:flex-row justify-between items-center gap-4 border-b border-slate-50">
            ...
            <div className="flex flex-wrap items-center justify-center lg:justify-end gap-3 w-full">
              ...
              <button
                ...
                className={`p-3 rounded-xl transition-all ${selectedIds.length === 0 || deleting
                ...
                {paginatedQuotations.map((qt) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-4 text-center">
                      <span className={getStatusStyle(qt.status)}>
                        {qt.status}
                      </span>
                    </td>
            ...
            {filteredQuotations.length === 0 && (
              <div className="py-32 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-slate-400 text-sm max-w-xs">
                  We couldn't find any quotations matching your current filter
                  criteria.
                </p>
              </div>
            )}
              ...
              <div className="flex items-center gap-6">
                <div className="text-[11px] font-bold text-slate-800 uppercase tracking-widest">
                  ...
                  <span className="text-slate-900">
                    {filteredQuotations.length > 0 ? startIndex + 1 : 0}
                  </span>{" "}
                  to{" "}
                  <span className="text-slate-900">
