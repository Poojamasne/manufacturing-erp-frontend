...
const Login: React.FC = () => {
  ...
  const passwordStatus = useMemo(() => {
    ...
    if (password.includes(" ")) return { error: "Spaces are not allowed", isValid: false };
    return { error: null, isValid: true };
  }, [password]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (
      email &&
      email.trim() === "planner@production.com" &&
      password &&
      password.trim() === "planner123"
    ) {
  ...
  return (
    <div className="h-screen flex items-center justify-center bg-[#f4f7f6] px-4">
      <div className="w-full max-w-4xl h-[90vh] bg-white rounded-2xl shadow-xl border border-gray-200 flex flex-col md:flex-row overflow-hidden">
        <div className="hidden md:flex md:w-1/2 bg-[#F59E0B] p-8 flex-col justify-between">
          ...
          <p className="text-[10px] text-[#f3f4e6] uppercase tracking-widest">Secure Access</p>
        </div>
