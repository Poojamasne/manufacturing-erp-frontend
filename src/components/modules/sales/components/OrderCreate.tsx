...
const OrderCreate: React.FC = () => {
    ...
    const { quotations } = useAppSelector((state: RootState) => state.SalesQuotation);
    ...
    const [errors, setErrors] = useState<Partial<Record<keyof OrderFormData, string>> & { lineItems?: string }>({});
    ...
    const handleSubmit = async () => {
        ...
        if (Object.keys(newErrors).length > 0) {
            ...
            } else if (newErrors.customer_name || newErrors.email || newErrors.phone || newErrors.shipping_address) {
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4">
                    <div className="flex items-center gap-4">
                        <button onClick={() => navigate('/sales/orders')} className="p-2 hover:bg-white rounded-xl transition-colors">
                            <ChevronLeft size={24} className="text-gray-400" />
                        </button>
                        <div>
                            <h1 className="text-2xl font-bold text-gray-800">Create New Order</h1>
                            <p className="text-sm text-gray-400 mt-1">Convert quotation to sales order</p>
                        </div>
                    </div>
                    <div className="flex gap-3 w-full lg:w-auto">
                        <button onClick={() => window.print()} className="flex-1 lg:flex-initial flex items-center justify-center gap-2 bg-white text-gray-600 px-5 py-2.5 rounded-xl font-medium text-sm border border-gray-200 hover:bg-gray-50">
                            <Printer size={18} /> Preview
                        </button>
                        <button onClick={handleSubmit} disabled={loading} className="flex-1 lg:flex-initial flex items-center justify-center gap-2 bg-[#005d52] text-white px-6 py-2.5 rounded-xl font-bold text-sm shadow-lg hover:bg-[#004a41] disabled:opacity-50">
                ...
                <div className="bg-white rounded-2xl p-2 mb-6 shadow-sm border border-gray-100">
                    <div className="flex flex-wrap gap-2">
                        {sections.map(section => (
                            <button key={section.id} onClick={() => setActiveSection(section.id)} className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-medium transition-all ${activeSection === section.id ? 'bg-[#d1e9e7] text-[#005d52]' : 'text-gray-500 hover:bg-gray-50'}`}>
                                <section.icon size={16} />
                                {section.label}
                                {(section.id === 'quotation' && errors.quotation_id) && <span className="w-2 h-2 bg-red-500 rounded-full"></span>}
                                {(section.id === 'customer' && (errors.customer_name || errors.email || errors.phone)) && <span className="w-2 h-2 bg-red-500 rounded-full"></span>}
                    ...
                    {activeSection === 'quotation' && (
                        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                            <div className="p-6 border-b border-gray-100">
                                ...
                                <p className="text-xs text-gray-400 mt-1">Choose a quotation to convert into an order</p>
                            </div>
                            <div className="p-6">
                                {errors.quotation_id && (
  ...
  .filter((quotation: any) =>
    ["sent", "accepted"].includes((quotation.status || "").toLowerCase())
  )
  .map((quotation: any) => (
                                            <div 
                                                key={quotation.id} 
                                                ...
                                                <div className="flex justify-between items-start">
                                                    <div className="flex-1">
                                                        <div className="flex items-center gap-3 mb-2">
                                                            <p className="font-bold text-gray-800">{quotation.quote_id || `QT-${quotation.id}`}</p>
                                                            <span className={getStatusBadge(quotation.status)}>
                                                                {quotation.status || 'Sent'}
                                                            </span>
                                                        </div>
                                                        <p className="text-sm text-gray-600 font-medium">{quotation.company_name}</p>
                                                        <div className="flex gap-4 mt-2 text-xs text-gray-400">
                                                            <span>Date: quotation.created_at
  ? new Date(quotation.created_at).toLocaleDateString()
  : "-"</span>
                                                            <span>Products: {quotation.products?.length || 0}</span>
                                                        </div>
                                                        <p className="text-sm font-bold text-[#005d52] mt-2">
                                                            Total: ₹{Number(quotation.total || 0).toLocaleString('en-IN')}
                                                        </p>
                                                    </div>
                                                    {selectedQuotation?.id === quotation.id && (
                                        ...
                                        <div className="text-center py-12 text-gray-400">
                                            ...
                                            <p className="text-sm font-medium">No quotations available</p>
                                            <p className="text-xs mt-1">Create a quotation first to convert it into an order</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                        ...
                        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                            ...
                            <div className="p-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <FormField label="Customer Name" value={formData.customer_name} onChange={(val) => setFormData({...formData, customer_name: val})} error={errors.customer_name} icon={<User size={16} />} required={true} />
                                    <FormField label="Email Address" type="email" value={formData.email} onChange={(val) => setFormData({...formData, email: val})} error={errors.email} icon={<Mail size={16} />} required={true} />
                                    <FormField label="Phone Number" value={formData.phone} onChange={(val) => setFormData({...formData, phone: val})} error={errors.phone} icon={<Phone size={16} />} required={true} />
                                    <FormField label="Shipping Address" type="textarea" value={formData.shipping_address} onChange={(val) => setFormData({...formData, shipping_address: val})} error={errors.shipping_address} icon={<MapPin size={16} />} required={true} />
                                    <FormField label="Additional Notes" type="textarea" rows={3} value={formData.notes} onChange={(val) => setFormData({...formData, notes: val})} icon={<FileText size={16} />} />
                                </div>
                            </div>
                        </div>
                    )}
                    ...
                    {activeSection === 'items' && (
                        <div className="bg-white rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
                            <div className="p-6 border-b border-gray-100 flex justify-between items-center">
                                ...
                                <button onClick={addLineItem} className="flex items-center gap-2 px-4 py-2 bg-[#d1e9e7] text-[#005d52] rounded-xl text-sm font-medium hover:bg-[#005d52] hover:text-white transition-all">
                                        ...
                                        {lineItems.map((item) => (
                                            <tr key={item.id} className="hover:bg-gray-50">
                                                        ...
                                                        onChange={(e) => {
                                                            setLineItems(prev => prev.map(i => {
                                                                if (i.id === item.id) {
                                                                    ...
                                                                    return { ...i, quantity: newQuantity, total: newQuantity * i.unit_price };
                                                                }
                                                                return i;
                                                            }));
                                                        }} 
                                                        ...
                                                        onChange={(e) => {
                                                            setLineItems(prev => prev.map(i => {
                                                                if (i.id === item.id) {
                                                                    ...
                                                                    return { ...i, unit_price: newPrice, total: i.quantity * newPrice };
                                                                }
                                                                return i;
                                                            }));
                                                        }} 
                                                ...
                                                <td className="p-4 text-center">
                                                    <button onClick={() => removeLineItem(item.id)} className="p-1 text-red-400 hover:text-red-600">
                                                        <Trash2 size={16} />
                                                    </button>
                                                </td>
                                            </tr>
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="border-t-2 border-gray-200 pt-4 mt-2">
                                    <div className="flex justify-between items-center">
                                        ...
                                        <span className="text-2xl font-bold text-[#005d52]">₹{totals.subtotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div className="mt-10 pt-6 border-t border-gray-200 flex flex-col sm:flex-row justify-end gap-3">
                    <button onClick={() => window.print()} className="flex items-center justify-center gap-2 bg-white text-gray-700 px-6 py-3 rounded-xl font-medium text-sm border border-gray-300 hover:bg-gray-50">
                        <Printer size={18} /> Preview Order
                    </button>
                    <button onClick={handleSubmit} disabled={loading} className="flex items-center justify-center gap-2 bg-[#005d52] text-white px-6 py-3 rounded-xl font-bold text-sm shadow-lg hover:bg-[#004a41] disabled:opacity-50">
                        {loading ? <Loader2 size={18} className="animate-spin" /> : <Save size={18} />}
                        {loading ? 'Creating...' : 'Create Order'}
                    </button>
                </div>
...
const FormField: React.FC<FormFieldProps> = ({ 
    label, value, onChange, error, icon, type = 'text', placeholder, rows = 3, required = false 
}) => (
