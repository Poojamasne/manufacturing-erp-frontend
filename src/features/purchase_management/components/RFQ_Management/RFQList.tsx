...
const StatusBadge: React.FC<{ status: string }> = ({ status }) => {
  const styles: { [key: string]: string } = {
    ...
    "Awaiting Response": "text-amber-600 bg-amber-50 border-amber-100",
...
const RFQList: React.FC = () => {
  ...
  const { rfqs } = useAppSelector((state: RootState) => state.rfqManagement);
  ...
  const filteredRFQs = useMemo(() => {
    ...
    if (searchQuery) {
      filtered = filtered.filter(
        (r) =>
          r.rfq_id.toLowerCase().includes(searchQuery.toLowerCase()) ||
  ...
  }, [rfqs, searchQuery, statusFilter, timeFilter, customRange]);

  const totalPages = Math.ceil(filteredRFQs.length / itemsPerPage);
  const paginatedData = filteredRFQs.slice(
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-10">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1 font-medium">
              Coordinate vendor biddings and price discovery
            </p>
          </div>
          <div className="flex items-center gap-3 w-full md:w-auto">
            ...
            <button
              onClick={() => navigate("/purchase/rfqs/create-rfq")}
        ...
        <div className="bg-white rounded-[2.5rem] shadow-sm border border-gray-100 overflow-hidden">
          <div className="p-6 flex flex-col lg:flex-row justify-between items-center gap-4 border-b border-slate-50">
            <div className="relative w-full lg:w-96">
              ...
              <input
                type="text"
                placeholder="Search RFQ or Material..."
                value={searchQuery}
            ...
            <div className="flex gap-3" ref={filterRef}>
              <div className="relative">
                ...
                {activeDropdown === "status" && (
                  <div className="absolute right-0 mt-2 w-26 bg-white rounded-2xl shadow-2xl z-50 py-2 border border-slate-50">
                    {[
                      "All",
                      "Draft",
                      "Sent",
                ...
                {paginatedData.length > 0 ? (
                    ...
                    >
                      ...
                      <td className="px-4 py-5 text-center">
                        <div className="flex items-center justify-center gap-1.5 bg-slate-50 py-1.5 rounded-xl border border-slate-100 px-3">
                          ...
                          <span className="text-[12px] font-bold text-slate-600">
                            {rfq.vendors.length} Suppliers
                          </span>
                        </div>
                      </td>
                      <td className="px-4 py-5 text-center text-[13px] font-bold text-slate-700">
                      ...
                      <td className="px-4 py-5">
                        <div className="flex justify-center gap-2">
                          <button
                            onClick={() =>
                              navigate("/purchase/rfqs/view-rfq/" + rfq.id)
                            }
                          ...
                          <button
                            onClick={() =>
                              navigate("/purchase/rfqs/edit-rfq/" + rfq.id)
                            }
                          ...
                          <button
                            onClick={() => dispatch(deleteRFQEntry(rfq.id))}
                  ...
                  <tr>
                    <td colSpan={7} className="py-20 text-center">
                      ...
                      />
                      <p className="text-slate-400 font-bold">No RFQs Found</p>
                    </td>
                  </tr>
                )}
              </tbody>
            ...
            <footer className="p-6 bg-slate-50/50 border-t border-slate-100 flex flex-col md:flex-row justify-between items-center gap-6">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="outline-none p-2.5 rounded-xl border border-slate-200 bg-white text-slate-400 hover:text-amber-500 hover:border-amber-500 disabled:opacity-30 transition-all shadow-sm"
                >
                  <ChevronLeft size={18} />
                </button>
                <div className="flex items-center gap-1.5">
                  {[...Array(totalPages)].map((_, i) => (
                    <button
                      ...
                      className={`outline-none min-w-10 h-10 rounded-xl text-xs font-bold transition-all shadow-sm ${currentPage === i + 1 ? "bg-[#F59E0B] text-white" : "bg-white text-slate-500 border border-slate-200"}`}
                    >
                      {i + 1}
                    </button>
                  ))}
