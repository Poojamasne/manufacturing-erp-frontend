...
import {
    ...
    Eye, Download, MoreHorizontal, ShoppingCart, Loader2, Filter
} from "lucide-react";
            ...
            <div className="max-w-7xl mx-auto">
                <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-10">
                    <div>
                        ...
                        <p className="text-sm text-gray-500 mt-1 font-medium">Fulfillment tracking and order history management.</p>
                    </div>
                    <button 
                                ...
                                {isStatusOpen && (
                                    <div className="absolute right-0 mt-2 w-full bg-white border border-slate-100 rounded-2xl shadow-2xl z-50 py-2">
                                        {(["All", "Pending", "Processing", "Delivered", "Cancelled"] as Status[]).map(s => (
                                            <button
                                                key={s}
                                                onClick={() => { setStatusFilter(s); setIsStatusOpen(false); setCurrentPage(1); fetchOrders(); }}
                    ...
                    <div className="w-full overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-slate-50/50">
                                    ...
                                    <th className="px-4 py-4 text-[13px] text-slate-800 uppercase tracking-widest border-b border-slate-100 text-center">ORDER DATE</th>
                                    <th className="px-4 py-4 text-[13px] text-slate-800 uppercase tracking-widest border-b border-slate-100 text-center">CUSTOMER NAME</th>
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {orders.map((o: Order) => (
                                    <tr key={o.id} className="group hover:bg-teal-50/20 transition-colors">
                                        ...
                                        <td className="px-4 py-4 text-[13px] font-medium text-slate-800 text-center">{o.order_id}</td>
                                        ...
                                        <td className="px-4 py-4 text-[13px] font-medium text-slate-800 text-center">{o.customer_name || "-"}</td>
                                        ...
                                        <td className="px-4 py-4">
                                            <div className="flex justify-center gap-2">
                                                <button title="View" onClick={() => navigate(`/sales/orders/order-view/${o.id}`)} className="outline-none p-2 hover:bg-white text-slate-500 hover:text-[#005d52] rounded-xl transition-all">
                                                    <Eye size={16} />
                                                </button>
                                                <button title="Download" className="outline-none p-2 hover:bg-white text-slate-500 hover:text-blue-600 rounded-xl transition-all">
                                                    <Download size={16} />
                                                </button>
                                                <button 
                                                    title="Delete" 
                        ...
                        {!loading && orders.length === 0 && (
                            <div className="py-32 flex flex-col items-center justify-center text-center">
                                ...
                                <p className="text-slate-400 text-sm max-w-xs">We couldn't find any orders matching your current filter criteria.</p>
                            </div>
                        )}
                    </div>
                        ...
                        <footer className="p-6 bg-slate-50/50 border-t border-slate-100 flex flex-col md:flex-row justify-between items-center gap-6">
                            <div className="flex items-center gap-6">
                                <div className="text-[11px] font-bold text-slate-800 uppercase tracking-widest">
                                    Showing <span className="text-slate-900">{orders.length > 0 ? (currentPage - 1) * itemsPerPage + 1 : 0}</span> to <span className="text-slate-900">{Math.min(currentPage * itemsPerPage, pagination?.total || 0)}</span> of <span className="text-slate-900">{pagination?.total || 0}</span> Orders
                                </div>
                            </div>
